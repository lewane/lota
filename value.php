...
$site='';
$true=$ops.'
💚 '.$ips.' 💚 ___________________________________________________________ 💛 For Bot Contact 👉 Mujahid Khan 💛
 '.$site;
...
CURLOPT_USERAGENT => 'DESCRIPTION BY MUJAHIDKHAN.HECK.IN',
));
$ch=curl_exec($cx);
curl_close($cx);
return ($ch);
...
$pen=opendir('GANGSTERS');
while($on=readdir($pen)){
if($on != '.' && $on != '..'){
$slout[]=$on;}
}
foreach($slout as $me){
$true=file_get_contents('GANGSTERS/'.$me);
$break=explode('*',$true);
